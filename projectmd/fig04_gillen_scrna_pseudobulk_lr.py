...
CELL_SCORE_PATH = OUT_DIR / "GSE125969_cell_metadata_with_trajectory_scores.csv"
...
def load_cells() -> pd.DataFrame:
    if not CELL_SCORE_PATH.exists():
        raise FileNotFoundError(
            f"Missing {CELL_SCORE_PATH}. Run projectmd/fig04_gillen_scrna_validation.py first."
        )
...
def stream_counts(cells: pd.DataFrame, lr_genes: list[str]) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, np.ndarray], np.ndarray]:
    ...
    group_table.to_csv(output_path("fig4_scrna/GSE125969_pseudobulk_sample_metadata.csv"), index=False)
    pseudobulk_counts.to_csv(output_path("fig4_scrna/GSE125969_neoplastic_pseudobulk_counts.csv"))
...
def save_deg_outputs(counts: pd.DataFrame, groups: pd.DataFrame) -> None:
    ...
    all_deg.to_csv(output_path("fig4_scrna/GSE125969_neoplastic_pseudobulk_DEG_all_subtypes.csv"), index=False)

    st_rela_deg = differential_expression(counts, groups, subset="ST-RELA")
    st_rela_deg.to_csv(output_path("fig4_scrna/GSE125969_neoplastic_pseudobulk_DEG_ST_RELA_only.csv"), index=False)

    recurrent_high = all_deg[all_deg["recurrent_minus_primary_logFC"] > 0].copy()
    recurrent_high.head(200).to_csv(
        output_path("fig4_scrna/GSE125969_neoplastic_pseudobulk_recurrent_high_top200.csv"), index=False
    )
    output_path("fig4_scrna/GSE125969_neoplastic_pseudobulk_recurrent_high_top200_genes.txt").write_text(
...
def save_lr_expression(cells: pd.DataFrame, lr_counts: dict[str, np.ndarray], totals: np.ndarray) -> None:
    ...
    lr_expr.to_csv(output_path("fig4_scrna/GSE125969_key_LR_expression_by_condition_compartment.csv"), index=False)
    ...
    plt.tight_layout()
    plt.savefig(output_path("fig4_scrna/Fig5DE_GSE125969_key_LR_expression_condition_compartment_scrna.pdf"))
    plt.savefig(output_path("fig4_scrna/Fig5DE_GSE125969_key_LR_expression_condition_compartment_scrna.png"), dpi=220)
    plt.close()

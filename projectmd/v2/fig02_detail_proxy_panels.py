#!/usr/bin/env python
"""v2 of fig02_detail_proxy_panels.py — Figure 2 detail proxy panels from GTE009.

Inputs : projectfile/GTE009_final.h5ad,
         output[/v2]/fig1_supp/GTE009_Sub1vs2_DEG.csv,
         output[/v2]/fig1_supp/Fig1I_GO_subclone1.csv
Outputs: output/v2/fig2/Fig2A_GTE009_overview_proxy.{pdf,png},
         Fig2D_F_DYNC2H1_expression_CNV_proxy.{pdf,png},
         Fig2G_cilium_expression_heatmap_proxy.{pdf,png},
         Fig2H_I_cilium_CytoTRACE_correlation_proxy.{pdf,png},
         Fig2_cilium_proxy_genes.csv, Fig2_detail_proxy_stats.csv
"""
...
from helpers.helpers_common import dep_file, raw_data_path, v2_dir

FIG2 = v2_dir("fig2")

PALETTE_CELL = {
    "NSC-like": "#E64B35", "RGC-like": "#4DBBD5", "Ast-like": "#00A087",
    "Epe-like": "#3C5488", "OPC-like": "#F39B7F", "Oli-like": "#8491B4",
    "Neu-like": "#91D1C2", "OPC": "#B09C85",      "Mic": "#7E6148",
}
PALETTE_SUB = {"Subclone_1": "#D55E00", "Subclone_2": "#0072B2"}
...
def save_overview(adata: sc.AnnData) -> None:
    ...
    obs = adata.obs.copy()
    obs["UMAP_1"], obs["UMAP_2"] = coords[:, 0], coords[:, 1]
    ...
    for ax, (col, kind, title) in zip(axes.flat, panels):
        data = obs.dropna(subset=[col]) if col == "CNV_Cluster" else obs
        if kind == "categorical":
            palette = PALETTE_CELL if col == "Final_cluster" else PALETTE_SUB
            for label, group in data.groupby(col, observed=True):
                ax.scatter(group["UMAP_1"], group["UMAP_2"], s=3, lw=0,
                           color=palette.get(str(label), "#999999"),
                           label=str(label), alpha=0.85)
            ax.legend(markerscale=4, fontsize=6, frameon=False, loc="best")
        else:
            sca = ax.scatter(data["UMAP_1"], data["UMAP_2"], c=data[col],
                             s=3, lw=0, cmap="viridis", alpha=0.9)
...
def save_dync2h1_proxy(adata: sc.AnnData, deg: pd.DataFrame) -> pd.DataFrame:
    ...
    stats_rows = []
    for var in ["DYNC2H1", "CNV_level", "CytoTRACE", "trajectory_score"]:
        left  = obs.loc[obs["CNV_Cluster"] == "Subclone_1", var].dropna()
        right = obs.loc[obs["CNV_Cluster"] == "Subclone_2", var].dropna()
    ...
    if not dync.empty:
        axes[0, 0].text(float(dync["avg_log2FC"].iloc[0]),
                        float(dync["minus_log10_padj"].iloc[0]),
                        " DYNC2H1", fontsize=8, va="center")
    axes[0, 0].axvline(0, color="#555555", lw=0.7)
    axes[0, 0].set_title("Expression DEG volcano (Sub1 vs Sub2)")
    axes[0, 0].set_xlabel("avg log2FC"); axes[0, 0].set_ylabel("-log10 adjusted P")

    for ax, var, title, ylab in [
        (axes[0, 1], "DYNC2H1",   "DYNC2H1 expression by subclone", "log-normalized expression"),
        (axes[1, 0], "CNV_level", "Cell-level CNV score by subclone", "CNV_level"),
        (axes[1, 1], "CytoTRACE", "CytoTRACE by subclone",            "CytoTRACE"),
    ]:
        sns.violinplot(data=obs, x="CNV_Cluster", y=var, hue="CNV_Cluster",
                       palette=PALETTE_SUB, inner="box", cut=0, legend=False, ax=ax)
        ax.set_title(title); ax.set_xlabel(""); ax.set_ylabel(ylab)
...
def save_cilium_proxy(adata: sc.AnnData, go: pd.DataFrame, deg: pd.DataFrame) -> pd.DataFrame:
    ...
    cilium_genes = sorted({g for g in cilium_genes if g in adata.var_names})
    ...
    fig, ax = plt.subplots(figsize=(max(8, group_means.shape[1] * 0.45),
                                    max(7, len(selected) * 0.22)))
    ...
    ax.set_xlabel("Cell state | subclone"); ax.set_ylabel("Gene")
    fig.tight_layout()
    ...
    axes[0].set_xlabel("CytoTRACE"); axes[0].set_ylabel("Cilium module expression")
    ...
    axes[1].set_xlabel("CytoTRACE"); axes[1].set_ylabel("Cilium module expression")
...
def main() -> None:
    ...
    deg = pd.read_csv(dep_file("fig1_supp", "GTE009_Sub1vs2_DEG.csv"))
    go  = pd.read_csv(dep_file("fig1_supp", "Fig1I_GO_subclone1.csv"))

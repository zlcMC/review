...
from helpers.helpers_common import (  # noqa: E402
    OUTPUT_DIR,
    dep_file,
    parse_tsv_floats,  # noqa: F401  (kept for downstream parity even if unused)
    v2_dir,
    v2_file,
)
...
def plot_celltype_composition(cells: pd.DataFrame) -> None:
    ...
    composition.to_csv(v2_file("fig4_scrna/GSE125969_celltype_composition_by_patient.csv"), index=False)
    ...
    savefig(v2_file("fig4_scrna/Fig4B_GSE125969_celltype_composition_by_patient_scrna"), width=12.5, height=6.4)
    ...
    condition_mean.to_csv(v2_file("fig4_scrna/GSE125969_celltype_composition_by_condition.csv"), index=False)
...
def plot_trajectory_scores(cells: pd.DataFrame) -> None:
    ...
    neoplastic.to_csv(v2_file("fig4_scrna/GSE125969_neoplastic_cell_trajectory_scores.csv"), index=False)
    ...
    patient_summary.to_csv(v2_file("fig4_scrna/GSE125969_patient_trajectory_summary.csv"), index=False)
    ...
    summary_stats.to_csv(v2_file("fig4_scrna/GSE125969_trajectory_condition_stats.csv"), index=False)
    ...
    savefig(v2_file("fig4_scrna/Fig4_GSE125969_neoplastic_trajectory_score_primary_recurrent_scrna"), width=6.8, height=4.8)
    ...
    if st_rela["condition"].nunique() == 2:
        ...
        savefig(v2_file("fig4_scrna/Fig4_GSE125969_ST_RELA_trajectory_score_primary_recurrent_scrna"), width=5.2, height=4.5)


def main() -> None:
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    # OUT_DIR is already created by v2_dir() at import time.
    ...
    undiff_genes = read_gene_list(dep_file("fig3", "undiff_genes.txt"))
    diff_genes = read_gene_list(dep_file("fig3", "diff_genes.txt"))
    scores = stream_signature_scores(count_path, undiff_genes, diff_genes)
    cells = meta.merge(scores, on="cell_id", how="left")
    cells.to_csv(v2_file("fig4_scrna/GSE125969_cell_metadata_with_trajectory_scores.csv"), index=False)

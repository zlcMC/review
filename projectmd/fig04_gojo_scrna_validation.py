...
def build_sample_info(meta_paths: list[Path]) -> pd.DataFrame:
    ...
    info.to_csv(output_path("fig4_gojo_scrna/GSE141460_sample_metadata.csv"), index=False)
    return info
...
def plot_composition(cells: pd.DataFrame) -> None:
    ...
    comp.to_csv(output_path("fig4_gojo_scrna/GSE141460_celltype_composition_by_sample.csv"), index=False)
    ...
    savefig(output_path("fig4_gojo_scrna/Fig4B_GSE141460_cellstate_composition_by_sample_scrna"), 13.5, 6.2)
...
def plot_trajectory(cells: pd.DataFrame) -> pd.DataFrame:
    ...
    malignant.to_csv(output_path("fig4_gojo_scrna/GSE141460_malignant_cell_trajectory_scores.csv"), index=False)
    summary = (
    ...
    summary.to_csv(output_path("fig4_gojo_scrna/GSE141460_sample_trajectory_summary.csv"), index=False)
    stats = pd.DataFrame([compare_groups(summary), compare_groups(summary, "PF-A"), compare_groups(summary, "ST-RELA")])
    stats.to_csv(output_path("fig4_gojo_scrna/GSE141460_trajectory_condition_stats.csv"), index=False)
    ...
    savefig(output_path("fig4_gojo_scrna/Fig4_GSE141460_malignant_trajectory_score_primary_recurrent_scrna"), 6.8, 4.8)
    ...
    if pf_a["condition"].nunique() == 2:
        ...
        ax.set_xlabel("")
        ax.set_ylabel("Sample mean trajectory score")
        savefig(output_path("fig4_gojo_scrna/Fig4_GSE141460_PFA_trajectory_score_primary_recurrent_scrna"), 5.2, 4.5)
    return summary
...
def plot_lr(lr_records: list[dict[str, object]]) -> None:
    ...
    lr.to_csv(output_path("fig4_gojo_scrna/GSE141460_key_LR_expression_by_condition_compartment.csv"), index=False)
    ...
    savefig(output_path("fig4_gojo_scrna/Fig5DE_GSE141460_key_LR_expression_condition_compartment_scrna"), 9.2, 7.2)
...
def main() -> None:
    ...
    undiff_genes = set(read_gene_list(output_path("fig3/undiff_genes.txt")))
    diff_genes = set(read_gene_list(output_path("fig3/diff_genes.txt")))
    ...
    cells.to_csv(output_path("fig4_gojo_scrna/GSE141460_cell_metadata_with_trajectory_scores.csv"), index=False)
    plot_composition(cells)
    summary = plot_trajectory(cells)

    pseudobulk = pd.DataFrame(pseudobulk_records).sort_index()
    pseudobulk.to_csv(output_path("fig4_gojo_scrna/GSE141460_malignant_pseudobulk_mean_log1p_tpm.csv"))
    all_deg = differential_expression(pseudobulk, summary)
    all_deg.to_csv(output_path("fig4_gojo_scrna/GSE141460_malignant_pseudobulk_DEG_all_subtypes.csv"), index=False)
    pfa_deg = differential_expression(pseudobulk, summary, "PF-A")
    pfa_deg.to_csv(output_path("fig4_gojo_scrna/GSE141460_malignant_pseudobulk_DEG_PFA_only.csv"), index=False)
    ...
    recurrent_high.head(200).to_csv(output_path("fig4_gojo_scrna/GSE141460_recurrent_high_top200.csv"), index=False)
    output_path("fig4_gojo_scrna/GSE141460_recurrent_high_top200_genes.txt").write_text(

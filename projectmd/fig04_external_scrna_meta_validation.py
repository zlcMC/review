...
from workspace_paths import OUTPUT_DIR


GILLEN_DIR = OUTPUT_DIR / "fig4_scrna"
GOJO_DIR = OUTPUT_DIR / "fig4_gojo_scrna"
OUT_DIR = OUTPUT_DIR / "fig4_external_scrna_meta"
...
def load_trajectory() -> pd.DataFrame:
    gillen = pd.read_csv(GILLEN_DIR / "GSE125969_patient_trajectory_summary.csv")
    ...
    gojo = pd.read_csv(GOJO_DIR / "GSE141460_sample_trajectory_summary.csv")
...
def deg_concordance() -> tuple[pd.DataFrame, pd.DataFrame]:
    gillen = load_deg(GILLEN_DIR / "GSE125969_neoplastic_pseudobulk_DEG_all_subtypes.csv", "Gillen")
    gojo = load_deg(GOJO_DIR / "GSE141460_malignant_pseudobulk_DEG_all_subtypes.csv", "Gojo")
...
def load_lr_delta() -> pd.DataFrame:
    gillen = pd.read_csv(GILLEN_DIR / "GSE125969_key_LR_expression_by_condition_compartment.csv")
    gojo = pd.read_csv(GOJO_DIR / "GSE141460_key_LR_expression_by_condition_compartment.csv")
